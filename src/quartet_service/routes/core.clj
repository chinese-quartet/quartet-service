(ns quartet-service.routes.core
  (:require [clojure.spec.alpha :as s]
            ...
            [quartet-service.routes.core-specs :as specs]
            [clojure.java.io :as io]
            [ring.util.http-response :refer [ok]]
            [ring.util.mime-type :as mime-type]
            [tservice-core.plugins.env :refer [get-workdir get-workdir-root]]
            [tservice-core.tasks.http :refer [get-manifest]]
            [local-fs.core :as fs-lib]
            [quartet-service.middleware.exception :as exception]
            [quartet-service.middleware.formats :as formats]
            [quartet-service.routes.task :as task-route]
            [remote-fs.route :as fs-route]
            [quartet-service.version :as v]
            [quartet-service.db.core :as db]))
...
(defn get-relative-filepath
  ...
  (let [replace-str (if filemode "." "")]
    (-> filepath
        (clj-str/replace (re-pattern (get-workdir-root)) replace-str)
        (clj-str/replace #"^\/" ""))))
...
(defn routes []
  ["/api"
   ...
   ["/version"
    {:tags ["Utility"]
     :get {:summary "Get the version of quartet-service instance."
           :parameters {}
           :responses {200 {:body specs/instance-version}}
           :handler (fn [_]
                      (ok {:version (v/get-version "quartet-service" "quartet-service")
     ...
     :get {:summary "Downloads a file"
           ...
           :handler (fn [{{{:keys [filelink]} :query} :parameters}]
                      {:status 200
                       ...
                       :body (-> (fs-lib/join-paths (get-workdir-root) (str "." filelink))
                                 (io/input-stream))})}}]
